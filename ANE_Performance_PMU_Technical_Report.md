...
- [`coreai_loader.m`](file:///Users/freedom/work/ios-hacking/ane_pmu_profiler/coreai_loader.m): High-performance Objective-C model loader:
  ...
  - **Direct Pre-Compiled Silicon Binding**: Loads directly from `/Library/Caches/com.apple.aned/.../<hash1>/<hash2>/model.hwx` (or locally supplied `.hwx`) into `_ANEClient` via `+[_ANEModel modelAtURL:key:]` in $1.29\text{ ms}$ (note: `/Library/Caches/com.apple.aned` is mode 0700 root-only on standard macOS; requires `sudo`, `chmod +rx`, or a local `--hwx` file).
